...
namespace SherdogComScraper.ScrapySharp
{
    public class SherdogScraperScrapySharp : ISherdogScraper
    {
        private readonly ScrapingBrowser _browser;
        private char[] _percent = new char[] { '%' };

        public SherdogScraperScrapySharp()
        {
            _browser = new ScrapingBrowser();
        }

        public SherdogScraperScrapySharp(ScrapingBrowser browser)
        {
            _browser = browser;
        }
        ...
        public async Task<IEnumerable<SherdogEvent>> Scrape()
        {
            ...
            WebPage homePage = await _browser.NavigateToPageAsync(
               uri);
            var tasks = homePage.Html.CssSelect(".event a")
        ...
        public async Task<SherdogEvent> ScrapeEvent(string url)
        {
            ...
            WebPage eventPage = await _browser.NavigateToPageAsync(uri);
        ...
        {
            var uri = new Uri(url);
            WebPage eventPage = await _browser.NavigateToPageAsync(
                uri);
